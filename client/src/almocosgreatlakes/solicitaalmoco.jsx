...
import { BASE_URL, CARDAPIO_URL, DELETA_COBRANCA, DELETE_URL_ESCOLHAS, ESCOLHAS_URL, GETALLKIDS, INSEREALMOCO, REGISTRA_COBRANCA, SENDEMAIL, SUPORTE_EMAILS } from "../functions/urlbase";
...
const SolicitaAlmoco = () => {
  ...
  //Funçao que vai buscar os dados do estudante se for um usuário 101 ou 130.

  function getAllKids() {

    fetch(BASE_URL+GETALLKIDS, {
      method: "GET",
      headers: {
  ...
  useEffect(() => {
    try {
      if (cookies.userData.userLevel === 101|| cookies.userData.userLevel === 130) {
        const listaDeEstudantes = getAllKids();
        //setEstudantes(listaDeEstudantes);
      }
      fetchCard();
      fetchEscolhas();
    } catch (err) {
